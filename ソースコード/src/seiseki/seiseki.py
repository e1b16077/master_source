...
# 重複を削除
purpo1 = list(set(test1))
purpo2 = list(set(test2))
purpo3 = list(set(test3))

# 頭文字がeから始まらないものを削除する処理
newlist1 = [x for x in purpo1 if str(x) != 'nan' and str(x) != 'None']
newlist2 = [x for x in purpo2 if str(x) != 'nan' and str(x) != 'None']
newlist3 = [x for x in purpo3 if str(x) != 'nan' and str(x) != 'None']
...
s2 = data_df3[data_df3['hsid'].isin(newlist2)]
s3 = data_df3[data_df3['hsid'].isin(newlist3)]

print(s1["SAチェック1"].to_list())
print("次")
print(s2["SAチェック2"].to_list())
print("次")
print(s3["SAチェック3"].to_list())

data = (s1["SAチェック1"].to_list(), s2["SAチェック2"].to_list(), s3["SAチェック3"].to_list())
